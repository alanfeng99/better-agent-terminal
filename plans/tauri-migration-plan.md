...
最近一次更新：2026-05-09 (#4)。（Phase 1 完成 17 個命名空間。Phase 2 已切三刀：(slice 1) JSON-RPC sidecar foundation；(slice 2) claude session lifecycle + event forwarding；(slice 3) claude 帳號 / metadata：sidecar 加 7 個 auth/account 方法 + 10 個 read-only metadata 方法（含 listSessions、getSupportedModels/Commands/Agents、getAccountInfo、getSessionState/Meta、getContextUsage、getWorktreeStatus、getCliPath），全部 stub 回 sensible defaults；adapter 加 3 個 event listeners (`onStream` / `onStatus` / `onModeChange`)。`host.claude` 已 port 29 個方法，剩下 ~15 個（auto-continue、permission/mode/sandbox/effort setters、resetSession、resumeSession、resolvePermission/AskUser、scanSkills、cleanupWorktree 等）下個 slice 處理。cargo test 100 例綠、host-api 8 情境（含 25+ claude invoke）、`pnpm test:sidecar`、Tauri release build & smoke 全綠。）
...
- [x] **首批 Rust commands**
  ...
  - `claude_get_cli_path` / `claude_list_sessions` / `claude_get_supported_models` / `claude_get_supported_commands` / `claude_get_supported_agents` / `claude_get_account_info` / `claude_get_session_state` / `claude_get_session_meta` / `claude_get_context_usage` / `claude_get_worktree_status`（Phase 2 slice 3；10 條 read-only metadata，全部回 null/[]/'' 直到實作上線。讓 renderer 每個 panel mount 都不會崩）。
