...
最近一次更新：2026-05-09 (#8)。（Phase 1 + Phase 2 全綠 + Phase 3 半完成：(a) sidecar 透過 `bundle.resources` 進 NSIS/MSI release exe；(b) `host.update.check` 透 sidecar fetch 拿 GitHub Releases；(c) remote/tunnel namespaces 全 stub-routed；(d) tauri-launch smoke test 看到 `thread '...' panicked` 直接 fail；(e) **新**：把兩條 read-only metadata 從 sidecar stub 換成真實實作 — `claude.getCliPath`（走 PATH + PATHEXT 找 `claude` binary）與 `claude.listSessions`（讀 `~/.claude/projects/<encoded-cwd>/*.jsonl`，前 20 行抓首條 user message 當 preview，max 50 sessions，dedup + mtime desc 排序），這兩條與 Anthropic SDK 沒耦合，所以不必等 SDK port 就能上線。剩下 Phase 3 expensive 的工作：(1) bundle Node runtime（pkg / bun --compile / Node SEA）；(2) 實打 `@anthropic-ai/claude-agent-sdk` 進 sidecar 把 stub 的 startSession/sendMessage 換成真實 Anthropic call；(3) 同樣對 OpenAI / worktree-manager / remote / tunnel 走真實實作。cargo test 104、host-api 8 情境、sidecar 測試（多兩條 findClaudeCliPath + listSessionsFallback 的 fixture-driven 測試）、Tauri release build & smoke 全綠。）
...
- [x] **首批 Rust commands**
  ...
  - `worktree_create` / `worktree_remove` / `worktree_status` / `worktree_merge` / `worktree_rehydrate`（Phase 2 slice 4；agent-tied worktree-manager.ts 還在 Electron，sidecar 全部回 `{success:false, error: 'sidecar stub'}`）。
